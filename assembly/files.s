...
#
# BUILDING:
#          as files.s -o files.o --32
#          ld -m elf_i386 files.o -o uppercase
#
# USE:
#          ./uppercase files.s files.up
#
...
.section .bss
 ...
 # the data file and written from into the outputfile.
...
open_fd_out:
 ...
 movl $0666, %edx                 # mode (if new file created)
 int $LINUX_SYSCALL               # call Linux
...
convert_to_upper:
 ...
 cmpl $0, %ebx  # if buffer has zero length, just leave
 je end_convert_loop
...
convert_loop:
 ...
 cmpb $LOWERCASE_Z, %cl
 jg next_byte
 addb $UPPER_CONVERSION, %cl # otherwise uppercase
 movb %cl, (%eax,%edi,1)     # and store it back

next_byte:
 incl %edi         # next byte
 cmpl %edi, %ebx   # continue unless we’ve reached end
 jne convert_loop
